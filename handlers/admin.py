...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, 'Что нужно?',
...
async def load_photo(message: types.Message, state: FSMContext):
    """Принимаем фото и записываем в словарь"""
    if message.from_user.id == ID:
...
async def load_name(message: types.Message, state: FSMContext):
    """Принимаем название и записываем в словарь"""
    if message.from_user.id == ID:
        async with state.proxy() as data:
...
async def load_description(message: types.Message, state: FSMContext):
    """Принимаем описание и записываем в словарь"""
    if message.from_user.id == ID:
        async with state.proxy() as data:
...
async def load_price(message: types.Message, state: FSMContext):
    """Принимаем цену и записываем в словарь"""
    if message.from_user.id == ID:
        async with state.proxy() as data:
...
def register_handlers_admin(dp: Dispatcher):
    ...
    dp.register_message_handler(make_changes_command, commands=['moderator'],
                                is_chat_admin=True)
    # dp.register_callback_query_handler(del_callback_run, lambda x: x.data and
